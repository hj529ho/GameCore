...
public static class Eulrlega {
    ...
    public static string FormatK(this string str, params object[] values)
    {
        ...
        {
            var token1 = string.Concat(".", t[0]);
            var token2 = string.Concat(".", t[1]);
            if (str.Contains(token1) || str.Contains(token2)) {
                var l = 0;
                while ((str.Contains(token1) || str.Contains(token2)) && l++ < 10000) {
                    // .? => *?
                    str = ProcessFormat(str, t[0], t, values);
                    str = ProcessFormat(str, t[1], t, values);
                }

                // *? => ?
                str = str.Replace(string.Concat("*", t[0]), t[0])
                    .Replace(string.Concat("*", t[1]), t[1]);
            }
        }
